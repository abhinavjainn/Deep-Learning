...
class Discriminator(nn.Module):
    ...
    def __init__(self, input_dims, output_dims=1):
        ...
        Declare the discriminator network structure
...
def im_convert(tensor):
  """
  Function for converting image tensor to numpy image.
...
def show_result(G_net, z_, num_epoch, show=False, save=False, path='result.png'):
    ...
    # Showing and saving images for epochs 10,20,50,100
    if num_epoch == 10 or num_epoch == 20 or num_epoch == 50 or   num_epoch == 100:  
      img_g = G_net(z_)
      img = torch.empty(1,28,28)
      fig = plt.figure(figsize=(5,5))
      for (i,j) in enumerate(img_g):
        img = j.view(1,28,28)
        if save:
          path = "./MNIST_GAN_results/results/MNIST_GAN_Epoch "+str(num_epoch)+"_Image "+str(i+1)+".png"
          save_image(img,path) 
        fig.add_subplot(5, 5, i+1, xticks=[], yticks=[])
        fig.suptitle("Epoch: "+str(num_epoch))
...
def show_train_hist(hist, show=False, save=False, path='Train_hist.png'):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.suptitle("Training Loss") # Set plot title

    plt.legend(loc=4)
    plt.grid(True)
    plt.tight_layout(3.0) # Add padding to avoid overlap
...
if __name__ == '__main__':
  ...
  # Additional code for MNIST download error
    ...
    show_train_hist(train_hist, show = True, save=True, path=save_dir + '/MNIST_GAN_train_hist.png')